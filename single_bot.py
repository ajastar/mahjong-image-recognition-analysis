...
# クリック設定
CLICK_CONFIDENCE_THRESHOLD = 0.5  # AIの自信が50%を超えたらクリック
ACTION_COOLDOWN = 1.5             # 1回操作したら何秒待つか
...
class MouseController:
    ...
    def click_at(self, x, y):
        ...
        time.sleep(random.uniform(0.05, 0.1)) # 人間らしい遅延
        # マウスアップ
        self.user32.mouse_event(0x0004, 0, 0, 0, 0)
        
    def click_tile(self, window, tile_obj):
        """
        検出された牌オブジェクト(nx, ny)をもとに
        ウィンドウ内の絶対座標を計算してクリック
        """
        # YOLOのnx, nyは 0.0~1.0 の正規化座標
        # ウィンドウの左上座標 + (幅 * 比率)
        
        # LDPlayerの黒帯除去などを考慮するならここを調整
        # 単純なリサイズと仮定
        target_x = window.left + (tile_obj['nx'] * window.width)
        target_y = window.top + (tile_obj['ny'] * window.height)
        
        # 少しランダムにずらす（誤BAN対策）
        offset_x = random.randint(-5, 5)
        offset_y = random.randint(-5, 5)
        
        self.click_at(target_x + offset_x, target_y + offset_y)
...
# 🛠️ 初期化
# ==========================================
print("⏳ Loading Models for Multi-Instance...")
model_main = None
...
class RobustGrid:
    ...
    def scan_hud_info(img_std, reader, matcher):
        ...
        areas = {
            "Round_Wind": (922, 414, 78, 30), "Info_TilesLeft": (948, 449, 50, 26),
    ...
    def _analyze_frame(self, img_bgr):
        ...
        if brain_model and len(hand) > 0:
            try:
                ...
                with torch.no_grad():
                    logits, naki_now = brain_model(t_now_batch)
                ...
                if candidates:
                    best_move = candidates[0]["tile"]
                    confidence = candidates[0]["base_conf"]
                    # ツモ切り指示補正
                    if best_move == "ツモ切り" and sum(hands_arr[3]) < 14 and len(candidates) > 1:
                         best_move = candidates[1]["tile"]
                         confidence = candidates[1]["base_conf"]
        ...
        print(f"[{self.title}] 打: {best_move} ({confidence:.1%}) {riichi_advice}")
        ...
        # クールダウンチェック & Wait以外 & 自信あり
        if (current_time - self.last_action_time > ACTION_COOLDOWN) and \
           (best_move != "Wait") and (confidence > CLICK_CONFIDENCE_THRESHOLD):
            
            # 1. 打牌対象の牌オブジェクトを探す
            target_obj = None
            
            if best_move == "ツモ切り":
                # ツモ切り指示なら、手牌の中で一番右(cxが大きい)の牌を選ぶ
                if hand_objs:
                    hand_objs.sort(key=lambda x: x['cx'])
                    target_obj = hand_objs[-1]
            else:
                # 指定された牌ラベルを探す (複数ある場合は、右端に近い方を優先=ツモ切りリスク回避)
                candidates_objs = [obj for obj in hand_objs if obj['label'] == best_move]
                if candidates_objs:
                    candidates_objs.sort(key=lambda x: x['cx'])
                    target_obj = candidates_objs[-1] # 一番右にある同種牌を切る
            
            # 2. クリック実行
            if target_obj:
                print(f"   ⚡ Clicking {target_obj['label']} on {self.title}")
                mouse_ctrl.click_tile(self.window, target_obj)
                self.last_action_time = current_time

# ==========================================
# 🔄 マルチインスタンス メインループ
# ==========================================
def multi_instance_loop():
    sct = mss()
    print(f"🔍 Searching for multiple windows with title containing '{TARGET_WINDOW_TITLE}'...")
    ...
    while True:
        try:
            ...
            if not all_windows:
                print("⚠️ No LDPlayer windows found.")
                time.sleep(2)
                continue
            ...
            for w in all_windows:
                ...
                if hwnd not in sessions:
                    print(f"✨ New Session: {w.title} (ID: {hwnd})")
                    sessions[hwnd] = MahjongSession(w)
            
            for old_hwnd in list(sessions.keys()):
                if old_hwnd not in current_hwnds:
                    print(f"🗑️ Session Closed: {sessions[old_hwnd].title}")
                    del sessions[old_hwnd]
            ...
            # 全セッションを順次処理
            for hwnd, session in sessions.items():
                try:
                    if session.window.isMinimized: continue
                    session.process(sct)
                except Exception as e:
                    print(f"⚠️ Error in session {session.title}: {e}")
...
if __name__ == '__main__':
    print("🚀 Auto Multi-Instance Mahjong AI Started.")
    # 管理者権限が必要な場合があります
    multi_instance_loop()